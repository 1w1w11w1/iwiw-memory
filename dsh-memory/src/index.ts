...
function makeDefinition(tools: MemoryTools) {
  return {
    ...
    memory_extract: defineTool({
      ...
      async execute(args) {
        const a = args as { message: string; context?: string };
    ...
    memory_list: defineTool({
      ...
      async execute(args) {
        const a = args as { priority?: "core" | "normal" | "archive"; limit?: number };
    ...
    memory_update: defineTool({
      ...
      async execute(args) {
        const a = args as { slug: string; description: string; body: string; priority?: string };
...
export const apply = async (ctx: Context, config: PluginConfig = {}) => {
  ...
  const fetchCoreText = async (): Promise<string> => {
    try {
      const list = (await tools.list({ priority: "core", limit: 50 })) as { items?: Array<{ slug: string; description: string; content: string }> };
  ...
  const sessionEvents = (ctx as any).events ?? ctx.scope?.events;
  if (sessionEvents?.on) {
    const off = sessionEvents.on("session/event", (session: any, ev: any) => {
      ...
      if (!/我|你|我|我[觉想爱需要]|偏好|喜欢|不喜欢|决定|计划|记住|健康|有.+?病|过敏/.test(content)) return;
      // 异步，不阻塞事件循环
...
function extractTextFromMessage(msg: any): string {
  if (!msg) return "";
  const blocks = (msg.content ?? msg.message?.content) as Array<{ type: string; text?: string }> | undefined;
  if (!Array.isArray(blocks)) return typeof msg === "string" ? msg : "";
  return blocks
