...
- 普通输入即对话。启动时自动注入常驻层（`MEMORY_AGENT_STANDING_LAYERS`，默认 `profile,rules`）记忆全文；每轮按话题检索相关记忆注入上下文。
...
`python -m memory_agent.mcp_server`（等价入口 `python -m memory_agent`）以 stdio 提供 17 个工具（供 DSH 等 harness 挂载）：

memory_remember / search_memories / list_memories / read_memory / memory_stats / touch_memories / memory_update / memory_archive / memory_delete / memory_merge / memory_history / memory_rollback / maintenance_review / pending_actions / pending_approve / pending_reject / run_consolidate

DSH 侧经 `dsh-iwiw-memory` 插件挂载（内核 MCP 子进程 + 上下文注入），见[插件 README](../dsh-iwiw-memory/README.md)。
