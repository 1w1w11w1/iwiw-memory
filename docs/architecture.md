...
┌─────────────────────────────────────────────────────────┐
│ 界面层                                                     │
│   chat.py        CLI 对话工作台（core 注入 + 话题检索 + 记忆工具） │
│   mcp_server.py  MCP 工具面（14 个工具，DSH 接入桥）           │
├─────────────────────────────────────────────────────────┤
│ 应用逻辑层                                                  │
...
- **chat.py**：唯一对话入口。启动时注入 core（必须载入）记忆全文；每轮对用户消息做话题检索并注入相关记忆；回复前运行记忆工具轮（模型自主调用 memory_remember/search/read/list 完成写入与检索，最多 TOOL_MAX_LOOPS 轮）；提供全套记忆管理命令。
- **mcp_server.py**：无界面工具面，覆盖记忆系统的完整能力（检索/CRUD/审批/回滚/维护），供 DSH 等外部 harness 调用。
...
- **llm.py**：Anthropic/OpenAI 两种兼容调用，支持多轮消息与流式；complete_with_tools 提供非流式工具调用轮（OpenAI-style function calling）。
- **config.py**：路径（data/memory.db 真源）、LLM、记忆工具轮、分级、检索、会话压缩参数，全部可经环境变量覆盖。
