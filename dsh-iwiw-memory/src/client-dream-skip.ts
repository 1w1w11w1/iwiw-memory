...
import { MOON_SVG, makeSkipMoonSvg, readSessionId } from './client-dream-icon'
import { subscribeDreamEvents } from './client-dream-events'

/** 注入项标记属性（清理与幂等锚点；data-iwiw-session-id 记录绑定会话）。 */
export const SKIP_ITEM_ATTR = 'data-iwiw-skip-item'
...
export function injectSkipItem(menu: Element, sessionId: string, host: SkipItemHost): HTMLElement | null {
  for (const old of Array.from(menu.querySelectorAll(`[${SKIP_ITEM_ATTR}]`))) {
    if (old.getAttribute('data-iwiw-session-id') === sessionId) return null // 已注入过，本菜单完成
