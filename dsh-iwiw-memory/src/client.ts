...
import { blocksToText, computeFoldGroups, computeInjectionGroups, foldLabel, formatInjectionClock, toolCallDetail, type FoldGroup, type InjectionGroup } from './client-fold'
...
} from './client-delegate-vanish'

import { applyDelegateNotices, computeDelegateNotices, startDelegateStateSync, type DelegateNotice } from './client-delegate-notice'

import { startDreamIconManager } from './client-dream-icon'

import { startDreamSkipManager } from './client-dream-skip'
...
const FOLDED_ATTR = 'data-iwiw-memory-folded'

const ANCHOR_ATTR = 'data-iwiw-memory-anchor'

const BODY_ATTR = 'data-iwiw-memory-body'

const INJ_ANCHOR_ATTR = 'data-iwiw-injection-anchor'

const INJ_BODY_ATTR = 'data-iwiw-injection-body'

const INJ_PROMPT_ATTR = 'data-iwiw-injection-prompt'
...
function attachDisclosure(rowEl: HTMLElement, label: string, text: string): void {
  ...
  if (root.querySelector(':scope > [data-iwiw-detail-body]') !== null) return // 已增强
...
function applyInjectionFold(
  ...
  for (const container of containers) {
    ...
    for (const group of groups) {
      ...
      const bubble = prompt.querySelector<HTMLElement>(':scope > [data-iwiw-inj-bubble]')

      if (bubble !== null && bubble.textContent !== group.userText) bubble.textContent = group.userText

      const timeLabel = prompt.querySelector<HTMLElement>(':scope > [data-iwiw-inj-actions] > [data-iwiw-inj-time]')
...
export function apply(ctx: any): () => void {
  ...
  // 时代的 `[data-iwiw-injection-prompt] > div` 背景）会以同等/更高特异性命中新 DOM
