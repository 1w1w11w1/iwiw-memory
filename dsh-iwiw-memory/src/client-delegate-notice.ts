 ...
 * 文本标记兜底（防旧数据/元数据丢失）。与主会话折叠轮（[iwiw-memory-reflect]/
 * [iwiw-memory-dream] 文本 marker）天然不撞——两者文本不同。
 *
 * 与 client-fold 同款纪律：
...
import { subscribeDreamEvents } from './client-dream-events'
...
const PLUGIN_NAME = 'dsh-iwiw-memory'

/** 隐藏原始行的 data 属性。 */
const HIDDEN_ATTR = 'data-iwiw-delegate-hidden'
/** 气泡锚点 data 属性（值=节点 key）。 */
const ANCHOR_ATTR = 'data-iwiw-delegate-anchor'
...
function delegateVariantOf(node: NoticeNodeLike): DelegateVariant | undefined {
  ...
  // 文本兜底：无元数据的旧数据。文本标记与折叠 marker（[iwiw-memory-reflect]）不同，不撞。
...
export function applyDelegateNotices(groups: readonly DelegateNotice[]): void {
  ...
  for (const container of containers) {
    ...
    for (const group of groups) {
      ...
      let bubble = anchor.querySelector<HTMLElement>(':scope > [data-iwiw-delegate-bubble]')
      if (bubble === null) {
