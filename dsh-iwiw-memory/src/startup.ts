...
export const makeDomainSpec = (scopeKey: string) => ({
 ...
 */
export function acquireStartupLock(dbKey: string): Promise<net.Server | null> {
...
export function releaseStartupLock(server: net.Server | null): Promise<void> {
  if (!server) return Promise.resolve();
  return new Promise((resolve) => server.close(() => resolve()));
}

export function scopeKeyOf(databaseId: string): string {
...
export function completedInWindow(event: any, from: number, to: number): boolean {
...
export function countCompletedTurns(events: any[], from: number, to: number): number {
  let n = 0;
  for (const e of events) if (completedInWindow(e, from, to)) n += 1;
  return n;
}
...
export function probeHost(sctx: any): HostCapabilities {
  if (!sctx?.storageDomain?.open) return { ok: false, reason: "storageDomain missing" };
  if (!sctx?.sessionQuery?.listSessions) return { ok: false, reason: "sessionQuery missing" };
  if (!sctx?.sessionQuery?.readSession) return { ok: false, reason: "sessionQuery.readSession missing" };
  if (!sctx?.sessionPersistence) return { ok: false, reason: "sessionPersistence missing" };
...
export async function readSessionLog(
  ...
  if (!id) throw new Error("session record missing id");
 ...
 *  不是用户会话所属工作区。 */
export async function listMainSessions(sctx: any, signal: AbortSignal): Promise<any[]> {
  const records = await sctx.sessionQuery.listSessions(signal);
  if (!Array.isArray(records)) throw new Error("sessionQuery.listSessions returned a non-array result");
  return records.filter((r: any) => r?.header?.origin !== "subagent");
}

/** cold 会话的 mtime 粗筛：文件没动过就不必完整解码。
 *  路径不可得返回 true（宁可多读，不可漏读）。 */
export async function coldLogFresh(
  sctx: any,
  record: any,
  since: number,
  signal: AbortSignal,
...
export interface StartupOptions {
  ...
  /** 固定窗口整理。err 非空表示未成功。 */
  consolidate: (sinceMs: number, untilMs: number) => Promise<{ ok: boolean; error?: string; reviewed?: number; changedRows?: number }>;
  /** 完成情况上报（供下一次对话一次性告知）。 */
...
export async function runStartup(sctx: any, options: StartupOptions): Promise<StartupOutcome> {
  const now = options.now ?? (() => Date.now());
  const assertActive = () => {
    if (options.signal.aborted) throw options.signal.reason ?? new Error("aborted");
  };
  if (options.signal.aborted) return { status: "failed", error: "aborted" };
  ...
  try {
    ...
    const state: StartupState = StateSchema.parse(domain.global.get());
    const cutoff = now();
    const save = async (next: StartupState) => {
      assertActive();
    ...
    if (state.lastRun !== null && cutoff < state.lastRun) {
      options.log.warn(
        `[dsh-iwiw-memory] clock regressed (lastRun=${state.lastRun} > now=${cutoff}); `
    ...
    for (const record of sessions) {
      if (options.signal.aborted) return { status: "failed", error: "aborted", from, cutoff };
    ...
    options.log.info(
      `[dsh-iwiw-memory] startup reconcile done: sessionsRead=${sessionsRead} completedTurns=${completedTurns} reviewed=${reviewed}`,
    );
