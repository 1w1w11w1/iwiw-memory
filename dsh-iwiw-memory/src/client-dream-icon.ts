...
export const DREAM_ICON_ATTR = 'data-iwiw-dreamed'
/** 呼吸灯月牙标记（dream 进行中）。 */
export const DREAMING_ATTR = 'data-iwiw-dreaming'
/** 灰调「月牙+斜杠」标记（已跳过梦境整理，v0.18.0）。 */
export const SKIPPED_ATTR = 'data-iwiw-skip-dream'
...
export function applyDreamIcons(states: ReadonlyMap<string, DreamIconState>, rows?: Iterable<DreamRowLike>): void {
  ...
  for (const row of all) {
    ...
    if (slot !== null) {
      // 状态槽位：只追加/只移除自有节点——ANY_ICON_SEL 只会命中我们 data-iwiw-*
