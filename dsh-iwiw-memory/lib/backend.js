 ...
 */
export function ensureRuntime(opts) {
    const cwd = opts.cwd?.trim() || bundledCwd;
    let python = opts.python?.trim() || "python";
    if (!existsSync(cwd))
        throw new Error(`[dsh-iwiw-memory] 内核目录不存在: ${cwd}`);
    if (depsOk(python, cwd))
        return { python, cwd };
    ...
    if (!existsSync(venvPython)) {
        ...
        if (v.status !== 0)
            throw new Error(`[dsh-iwiw-memory] venv 创建失败: ${v.stderr?.slice(0, 300)}`);
    }
    if (!depsOk(venvPython, cwd)) {
        ...
        const p = spawnSync(venvPython, ["-m", "pip", "install", "--no-cache-dir", "-r", join(cwd, "requirements.txt")], { encoding: "utf8", timeout: 600_000 });
        if (p.status !== 0) {
    ...
    if (!depsOk(venvPython, cwd))
        throw new Error("[dsh-iwiw-memory] 内核依赖自举失败（venv 内仍缺 mcp/jieba）");
