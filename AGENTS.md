...
本项目是一个**独立的本地长期记忆系统**：核心为 SQLite 标签记忆（profile/fact/lesson/rules/project × active/archived），自带 CLI 对话工作台（`memory_agent/chat.py`）作为使用与验证界面，并以 MCP（`memory_agent/mcp_server.py`）对外提供完整工具面。
...
2. 记忆分类为五层类型（profile/fact/lesson/rules/project）× 二态生命周期（active=在役 / archived=归档留痕）；profile 与 rules 常驻注入（启动全量注入），其余按话题检索召回；archived 不参与常规检索与维护候选。
...
8. 语义检索必须进入真实对话上下文链路（chat 每轮注入 + 常驻层启动注入），不能只接在展示层。
9. 不确定是否属于稳定事实时，优先生成候选或保持观察，不要急着写入。
10. 用户明确要求删除、纠正或更新记忆时，以用户最近一次明确更正为准。

## 检索策略

1. 启动时仅加载常驻层（profile/rules）完整内容。
