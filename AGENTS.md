...
11. 写入闸门：凭据类（API key/token/密码/私钥/连接串口令）与公网 IP 在落库前由 `redaction.sanitize` 确定性脱敏，命中详情进 `memory_audit`；回环与内网地址（127.0.0.1、10/172.16-31/192.168/169.254/100.64 段）保留。脱敏必须幂等，且只能走统一 mutation 入口，不得在调用方各写一份。
...
7. 跨项目排查记忆问题时：在出问题的会话里用 DSH 头部导出按钮或 `/export` 导出（得到整会话 ZIP），把包放进 `debug-inbox/`（已 gitignore），运行 `python -m memory_agent.debug_bundle` 得到逐轮事实链（注入/命中、memory_* 调用与结果、报错；默认脱敏，`--raw` 保真）。
